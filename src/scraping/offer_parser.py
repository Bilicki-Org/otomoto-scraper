...
class OfferParser:
    ...
    Hybrid approach with enhanced location cleaning logic.
    """
    ...
    def _extract_location_html(self, soup: BeautifulSoup) -> Dict[str, str]:
        ...
        Includes 'Hardcoded Mapping' and regex fixes for zip codes without dashes.
        """
        loc_data = {"location_city": None, "location_district": None, "location_voivodeship": None}
        
        # Słownik mapujący miasta na województwa (Normalized Data)
        MAJOR_CITIES_MAP = {
        ...
        def clean_name(text: Optional[str]) -> Optional[str]:
            ...
            # 1. Usuń kody pocztowe (XX-XXX lub XX XXX)
            text = re.sub(r'\d{2}[-\s]\d{3}', '', text)
            
            # 2. Usuń "Polska"
            text = re.sub(r'Polska\s*[-–]?\s*', '', text, flags=re.IGNORECASE)
            
            # 3. Usuń prefiks "gm." (gmina)
            text = re.sub(r'^gm\.\s*', '', text, flags=re.IGNORECASE)
            
            # 4. Usuń nawiasy
            text = text.replace('(Polska)', '').replace('()', '')
            
            # 5. Usuń interpunkcję z początku i końca
            text = re.sub(r'^[\W_]+|[\W_]+$', '', text).strip()
            
            # 6. Obsługa "Miasto - Miasto" (duplikaty po myślniku)
            if ' - ' in text:
                parts = text.split(' - ')
                # Jeśli pierwsza i druga część są takie same (np. Wałbrzych - Wałbrzych)
        ...
        try:
            ...
            if map_header:
                ...
                if parent_div:
                    ...
                    location_tag = link_tag if link_tag else None
                    if not location_tag:
                        # Fallback
            ...
            if location_tag:
                ...
                # --- STRATEGIA 1: KOD POCZTOWY (PRIORYTET) ---
                # Zmieniony regex: \d{2}[-\s]\d{3} łapie "07-410" oraz "07 410"
            ...
            # --- STRATEGIA 3: WZBOGACANIE DANYCH (MAPPING) ---
